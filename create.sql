...
create table HFJ_FORCED_ID (PARTITION_DATE date, PARTITION_ID integer, PID bigint not null, RESOURCE_PID bigint not null, FORCED_ID varchar(100) not null, RESOURCE_TYPE varchar(100) default '', primary key (PID), constraint IDX_FORCEDID_RESID unique (RESOURCE_PID), constraint IDX_FORCEDID_TYPE_FID unique (RESOURCE_TYPE, FORCED_ID));
create table HFJ_HISTORY_TAG (PARTITION_DATE date, PARTITION_ID integer, PID bigint not null, RES_ID bigint not null, RES_VER_PID bigint not null, TAG_ID bigint, RES_TYPE varchar(40) not null, primary key (PID), constraint IDX_RESHISTTAG_TAGID unique (RES_VER_PID, TAG_ID));
create table HFJ_IDX_CMB_TOK_NU (PARTITION_DATE date, PARTITION_ID integer, HASH_COMPLETE bigint not null, PID bigint not null, RES_ID bigint, IDX_STRING varchar(500) not null, primary key (PID));
create table HFJ_IDX_CMP_STRING_UNIQ (PARTITION_DATE date, PARTITION_ID integer, PID bigint not null, RES_ID bigint, IDX_STRING varchar(500) not null, primary key (PID), constraint IDX_IDXCMPSTRUNIQ_STRING unique (IDX_STRING));
create table HFJ_RES_LINK (PARTITION_DATE date, PARTITION_ID integer, PID bigint not null, SP_UPDATED timestamp(6), SRC_RESOURCE_ID bigint not null, TARGET_RESOURCE_ID bigint, TARGET_RESOURCE_VERSION bigint, SOURCE_RESOURCE_TYPE varchar(40) not null, TARGET_RESOURCE_TYPE varchar(40) not null, TARGET_RESOURCE_URL varchar(200), SRC_PATH varchar(500) not null, primary key (PID));
create table HFJ_RES_PARAM_PRESENT (PARTITION_DATE date, PARTITION_ID integer, SP_PRESENT boolean not null, HASH_PRESENCE bigint, PID bigint not null, RES_ID bigint not null, primary key (PID));
create table HFJ_RES_SEARCH_URL (CREATED_TIME timestamp(6) not null, RES_ID bigint not null, RES_SEARCH_URL varchar(768) not null, primary key (RES_SEARCH_URL));
create table HFJ_RES_TAG (PARTITION_DATE date, PARTITION_ID integer, PID bigint not null, RES_ID bigint, TAG_ID bigint, RES_TYPE varchar(40) not null, primary key (PID), constraint IDX_RESTAG_TAGID unique (RES_ID, TAG_ID));
create table HFJ_RES_VER (HAS_TAGS boolean not null, PARTITION_DATE date, PARTITION_ID integer, RES_ENCODING varchar(5) not null check (RES_ENCODING in ('JSON','JSONC','DEL','ESR')), RES_VERSION varchar(7) check (RES_VERSION in ('DSTU2','DSTU2_HL7ORG','DSTU2_1','DSTU3','R4','R4B','R5')), PID bigint not null, RES_DELETED_AT timestamp(6), RES_ID bigint not null, RES_PUBLISHED timestamp(6) not null, RES_UPDATED timestamp(6) not null, RES_VER bigint not null, REQUEST_ID varchar(16), RES_TYPE varchar(40) not null, SOURCE_URI varchar(100), RES_TEXT_VC clob, RES_TEXT blob, primary key (PID), constraint IDX_RESVER_ID_VER unique (RES_ID, RES_VER));
create table HFJ_RES_VER_PROV (PARTITION_DATE date, PARTITION_ID integer, RES_PID bigint not null, RES_VER_PID bigint not null, REQUEST_ID varchar(16), SOURCE_URI varchar(100), primary key (RES_VER_PID));
create table HFJ_RESOURCE (HAS_TAGS boolean not null, PARTITION_DATE date, PARTITION_ID integer, SEARCH_URL_PRESENT boolean, SP_CMPSTR_UNIQ_PRESENT boolean, SP_CMPTOKS_PRESENT boolean, SP_COORDS_PRESENT boolean, SP_DATE_PRESENT boolean, SP_HAS_LINKS boolean, SP_NUMBER_PRESENT boolean, SP_QUANTITY_NRML_PRESENT boolean, SP_QUANTITY_PRESENT boolean, SP_STRING_PRESENT boolean, SP_TOKEN_PRESENT boolean, SP_URI_PRESENT boolean, RES_VERSION varchar(7) check (RES_VERSION in ('DSTU2','DSTU2_HL7ORG','DSTU2_1','DSTU3','R4','R4B','R5')), RES_DELETED_AT timestamp(6), RES_ID bigint not null, RES_PUBLISHED timestamp(6) not null, RES_UPDATED timestamp(6) not null, RES_VER bigint, SP_INDEX_STATUS bigint, RES_LANGUAGE varchar(20), RES_TYPE varchar(40) not null, FHIR_ID varchar(64), HASH_SHA256 varchar(64), primary key (RES_ID));
create table HFJ_RESOURCE_MODIFIED (CREATED_TIME timestamp(6) not null, RES_VER varchar(8) not null, RESOURCE_TYPE varchar(40) not null, RES_ID varchar(256) not null, SUMMARY_MESSAGE varchar(4000) not null, primary key (RES_VER, RES_ID));
create table HFJ_SPIDX_COORDS (PARTITION_DATE date, PARTITION_ID integer, SP_LATITUDE float(53), SP_LONGITUDE float(53), SP_MISSING boolean not null, HASH_IDENTITY bigint, RES_ID bigint not null, SP_ID bigint not null, SP_UPDATED timestamp(6), RES_TYPE varchar(100) not null, SP_NAME varchar(100) not null, primary key (SP_ID));
create table HFJ_SPIDX_DATE (PARTITION_DATE date, PARTITION_ID integer, SP_MISSING boolean not null, SP_VALUE_HIGH_DATE_ORDINAL integer, SP_VALUE_LOW_DATE_ORDINAL integer, HASH_IDENTITY bigint, RES_ID bigint not null, SP_ID bigint not null, SP_UPDATED timestamp(6), SP_VALUE_HIGH timestamp(6), SP_VALUE_LOW timestamp(6), RES_TYPE varchar(100) not null, SP_NAME varchar(100) not null, primary key (SP_ID));
create table HFJ_SPIDX_NUMBER (PARTITION_DATE date, PARTITION_ID integer, SP_MISSING boolean not null, SP_VALUE numeric(38,2), HASH_IDENTITY bigint, RES_ID bigint not null, SP_ID bigint not null, SP_UPDATED timestamp(6), RES_TYPE varchar(100) not null, SP_NAME varchar(100) not null, primary key (SP_ID));
create table HFJ_SPIDX_QUANTITY (PARTITION_DATE date, PARTITION_ID integer, SP_MISSING boolean not null, SP_VALUE float(53), HASH_IDENTITY bigint, HASH_IDENTITY_AND_UNITS bigint, HASH_IDENTITY_SYS_UNITS bigint, RES_ID bigint not null, SP_ID bigint not null, SP_UPDATED timestamp(6), RES_TYPE varchar(100) not null, SP_NAME varchar(100) not null, SP_SYSTEM varchar(200), SP_UNITS varchar(200), primary key (SP_ID));
create table HFJ_SPIDX_QUANTITY_NRML (PARTITION_DATE date, PARTITION_ID integer, SP_MISSING boolean not null, SP_VALUE float(53), HASH_IDENTITY bigint, HASH_IDENTITY_AND_UNITS bigint, HASH_IDENTITY_SYS_UNITS bigint, RES_ID bigint not null, SP_ID bigint not null, SP_UPDATED timestamp(6), RES_TYPE varchar(100) not null, SP_NAME varchar(100) not null, SP_SYSTEM varchar(200), SP_UNITS varchar(200), primary key (SP_ID));
create table HFJ_SPIDX_STRING (PARTITION_DATE date, PARTITION_ID integer, SP_MISSING boolean not null, HASH_EXACT bigint, HASH_IDENTITY bigint, HASH_NORM_PREFIX bigint, RES_ID bigint not null, SP_ID bigint not null, SP_UPDATED timestamp(6), RES_TYPE varchar(100) not null, SP_NAME varchar(100) not null, SP_VALUE_EXACT varchar(200), SP_VALUE_NORMALIZED varchar(200), primary key (SP_ID));
create table HFJ_SPIDX_TOKEN (PARTITION_DATE date, PARTITION_ID integer, SP_MISSING boolean not null, HASH_IDENTITY bigint, HASH_SYS bigint, HASH_SYS_AND_VALUE bigint, HASH_VALUE bigint, RES_ID bigint not null, SP_ID bigint not null, SP_UPDATED timestamp(6), RES_TYPE varchar(100) not null, SP_NAME varchar(100) not null, SP_SYSTEM varchar(200), SP_VALUE varchar(200), primary key (SP_ID));
create table HFJ_SPIDX_URI (PARTITION_DATE date, PARTITION_ID integer, SP_MISSING boolean not null, HASH_IDENTITY bigint, HASH_URI bigint, RES_ID bigint not null, SP_ID bigint not null, SP_UPDATED timestamp(6), RES_TYPE varchar(100) not null, SP_NAME varchar(100) not null, SP_URI varchar(500), primary key (SP_ID), constraint IDX_SP_URI_HASH_URI_V2 unique (HASH_URI, RES_ID, PARTITION_ID), constraint IDX_SP_URI_HASH_IDENTITY_V2 unique (HASH_IDENTITY, SP_URI, RES_ID, PARTITION_ID));
create table HFJ_TAG_DEF (TAG_TYPE tinyint not null check (TAG_TYPE between 0 and 2), TAG_USER_SELECTED boolean, TAG_ID bigint not null, TAG_VERSION varchar(30), TAG_CODE varchar(200), TAG_DISPLAY varchar(200), TAG_SYSTEM varchar(200), primary key (TAG_ID));
...
alter table if exists HFJ_FORCED_ID add constraint FK_FORCEDID_RESOURCE foreign key (RESOURCE_PID) references HFJ_RESOURCE;
...
alter table if exists HFJ_HISTORY_TAG add constraint FK_HISTORYTAG_HISTORY foreign key (RES_VER_PID) references HFJ_RES_VER;
alter table if exists HFJ_IDX_CMB_TOK_NU add constraint FK_IDXCMBTOKNU_RES_ID foreign key (RES_ID) references HFJ_RESOURCE;
alter table if exists HFJ_IDX_CMP_STRING_UNIQ add constraint FK_IDXCMPSTRUNIQ_RES_ID foreign key (RES_ID) references HFJ_RESOURCE;
alter table if exists HFJ_RES_LINK add constraint FK_RESLINK_SOURCE foreign key (SRC_RESOURCE_ID) references HFJ_RESOURCE;
alter table if exists HFJ_RES_LINK add constraint FK_RESLINK_TARGET foreign key (TARGET_RESOURCE_ID) references HFJ_RESOURCE;
alter table if exists HFJ_RES_PARAM_PRESENT add constraint FK_RESPARMPRES_RESID foreign key (RES_ID) references HFJ_RESOURCE;
alter table if exists HFJ_RES_TAG add constraint FKbfcjbaftmiwr3rxkwsy23vneo foreign key (TAG_ID) references HFJ_TAG_DEF;
alter table if exists HFJ_RES_TAG add constraint FK_RESTAG_RESOURCE foreign key (RES_ID) references HFJ_RESOURCE;
alter table if exists HFJ_RES_VER add constraint FK_RESOURCE_HISTORY_RESOURCE foreign key (RES_ID) references HFJ_RESOURCE;
alter table if exists HFJ_RES_VER_PROV add constraint FK_RESVERPROV_RES_PID foreign key (RES_PID) references HFJ_RESOURCE;
alter table if exists HFJ_RES_VER_PROV add constraint FK_RESVERPROV_RESVER_PID foreign key (RES_VER_PID) references HFJ_RES_VER;
alter table if exists HFJ_SPIDX_COORDS add constraint FKC97MPK37OKWU8QVTCEG2NH9VN foreign key (RES_ID) references HFJ_RESOURCE;
alter table if exists HFJ_SPIDX_DATE add constraint FK_SP_DATE_RES foreign key (RES_ID) references HFJ_RESOURCE;
alter table if exists HFJ_SPIDX_NUMBER add constraint FK_SP_NUMBER_RES foreign key (RES_ID) references HFJ_RESOURCE;
alter table if exists HFJ_SPIDX_QUANTITY add constraint FK_SP_QUANTITY_RES foreign key (RES_ID) references HFJ_RESOURCE;
alter table if exists HFJ_SPIDX_QUANTITY_NRML add constraint FK_SP_QUANTITYNM_RES foreign key (RES_ID) references HFJ_RESOURCE;
alter table if exists HFJ_SPIDX_STRING add constraint FK_SPIDXSTR_RESOURCE foreign key (RES_ID) references HFJ_RESOURCE;
alter table if exists HFJ_SPIDX_TOKEN add constraint FK_SP_TOKEN_RES foreign key (RES_ID) references HFJ_RESOURCE;
alter table if exists HFJ_SPIDX_URI add constraint FKGXSREUTYMMFJUWDSWV3Y887DO foreign key (RES_ID) references HFJ_RESOURCE;
create sequence SEQ_FORCEDID_ID start with 1 increment by 50;
create sequence SEQ_HISTORYTAG_ID start with 1 increment by 50;
create sequence SEQ_IDXCMBTOKNU_ID start with 1 increment by 50;
create sequence SEQ_IDXCMPSTRUNIQ_ID start with 1 increment by 50;
